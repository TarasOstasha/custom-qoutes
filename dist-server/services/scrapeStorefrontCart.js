...
async function openVisibleVolusionHomepageSession() {
    ...
    const userDataDir = (0, node_path_1.resolve)(profileDir);
    await (0, promises_2.mkdir)(userDataDir, { recursive: true });
    // Ensure visible browser. If a context already exists (possibly headless), restart it.
    if (liveContext) {
...
async function scrapeVolusionStorefrontCart(options = {}) {
    ...
    await page
        .waitForSelector("#v65-cart-total-estimate-cell, select[name='ShippingSpeedChoice'], #v65-cart-table", {
        state: "attached",
        timeout: 20000,
    })
        .catch(() => {
