...
function extractShippingSpeedChoiceInBrowser() {
    ...
    function isPleaseSelectOption(option) {
        if (!option)
            return true;
    ...
    function findSelectedOption(select) {
        ...
        for (const option of options) {
            if (optionIsMarkedSelected(option) && !isPleaseSelectOption(option))
                return option;
        }
        const indexed = select.selectedIndex >= 0 ? select.options.item(select.selectedIndex) : null;
        if (indexed && !isPleaseSelectOption(indexed))
            return indexed;
        return null;
    }
    ...
    Array.from(select.options).forEach((option) => {
        if (isPleaseSelectOption(option))
            return;
    ...
    if (selectedDom) {
        ...
        if (selectedValue && selectedValue !== "0") {
            let match = shippingOptions.find((option) => option.value === selectedValue) ??
...
async function scrapeShippingSpeedChoiceFromPage(page) {
    const select = page.locator(exports.SHIPPING_SPEED_SELECT_SELECTOR).first();
    ...
    return select.evaluate((el) => {
        ...
        function isMarkedSelected(option) {
            if (option.selected)
                return true;
            if (option.hasAttribute("selected"))
                return true;
        ...
        if (!options.length)
            return null;
        let selected = Array.from(el.options).find((option) => isMarkedSelected(option) && !isPleaseSelect(option));
...
async function enrichPayloadWithShippingSpeedChoice(page, payload) {
