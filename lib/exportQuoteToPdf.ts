...
async function loadImageDataUrl(src: string): Promise<string | null> {
  try {
    ...
    try {
      const proxied = await fetch(`${apiBase}/quotes/image-proxy?url=${encodeURIComponent(src)}`);
...
export async function exportQuoteToPdf(quote: Quote): Promise<void> {
  ...
  const tableEndY = ((doc as unknown as { lastAutoTable?: { finalY?: number } }).lastAutoTable?.finalY ?? y) + 8;
